...
export default function Scoreboard({ teams, compact = false }: { teams: Team[]; compact?: boolean }) {
  return (
    <div className={`flex items-stretch gap-3 ${compact ? '' : 'w-full max-w-3xl'}`}>
      {teams.map((team) => (
        <div
          key={team.id}
          className="flex-1 rounded-xl border border-arena-600 bg-arena-800/80 px-5 py-3 text-center shadow-lg shadow-black/30"
